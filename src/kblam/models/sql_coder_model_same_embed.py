...
PADDING_VALUE = torch.finfo(torch.float16).min
...
class KblamLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("self.config.pretraining_tp",self.config.pretraining_tp)
        ...
        # print("Here KEY States shape:",key_states.shape)
        # print("Here Value States shape:",value_states.shape)
        # print("Q_len:",q_len)
        ...
        if kb_kvs is not None:
            if self.layer_idx % kb_layer_frequency == 0:
                ...
                if len(kb_keys.shape) == 2:  # Not batch dim
                    ...
                    # print("KB keys shape:",kb_keys.shape)
                    # kb_len = kb_keys.shape[1]
                    # total_elements = kb_keys.numel()
                    # known_dimensions = bsz * kb_len * (1 + self.config.num_hidden_layers // kb_layer_frequency)
                    # missing_dim = total_elements // known_dimensions
                    # kb_keys = kb_keys.view(
                    #     bsz,
                    #     kb_len,
                    #     1 + self.config.num_hidden_layers // kb_layer_frequency,
                    ...
                    kb_keys = kb_keys.to('cuda')
                    kb_values =  kb_keys.to('cuda')
                    kb_len = kb_keys.shape[1]
                    kb_keys = kb_keys.view(
                            bsz, kb_len, self.num_heads, self.head_dim
                        ).transpose(1, 2)
                    ...
                    # print("kb_keys.shape",kb_keys.shape)
            ...
            if kb_kvs is not None:
                if self.layer_idx % kb_layer_frequency == 0:
                    ...
                    if attn_weights_2 is None:
                        query_states_2 = query_states_2.to(torch.float16)
                        kb_keys = kb_keys.to(torch.float16)
