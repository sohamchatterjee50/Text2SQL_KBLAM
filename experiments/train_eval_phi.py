...
from kblam.models.phi4_model_large import KBLaMPhi3ForCausalLM
...
parser.add_argument("--train_dataset",type=str,default="random_dev_bird_same_key_val_90",choices=["random_dev_bird_same_key_val_90","random_dev_bird_ST_90","train_dev_bird_naive_100", "random_dev_bird_naive_90"])
...
parser.add_argument("--hf_model_spec", type=str, default="microsoft/Phi-4-mini-instruct", choices=["microsoft/phi-4","microsoft/Phi-4-mini-instruct","meta-llama/Meta-Llama-3-8B", "microsoft/Phi-3-mini-4k-instruct", "meta-llama/Llama-3.2-1B-Instruct"])
parser.add_argument("--hf_token", type=str,default="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",help="Huggingface token")
parser.add_argument("--model_save_dir", type=str, default="/scratch-shared/chatty/phi4", help="Place to save the checkpoints")
...
def get_batch(
    qa_format_func: Callable[[str], str],
    dataset: List[Dict],
    tokenizer,
    ...
    batch_indices = np.arange(start_index, min(1376,start_index + B))
    ...
    with torch.autograd.no_grad():
        ...
        for idx in batch_indices:
            ...
            input_strs.append(qa_format_func(Q))
            answers.append(A)
    ...
    return input_ids, attention_masks,answers, batch_indices
...
class Trainer:
    ...
    def train(
        ...
        with create_custom_progress_bar(console=console) as pbar:
            task = pbar.add_task("Evaluating", total=self.num_steps,loss=-1)
            for step in range(start_step, self.num_steps, 1):
                input_ids, attention_masks, answers, batch_indices = self._get_batch(
                        training_set,
                        self.tokenizer,
                        self.device,
                ...
                print("Start Index:",prev_start_index)
                ...
                for batch_index in range(len(batch_indices)):
                    ...
                    true_answer = answers[batch_index]
                    
                    print("******************************************************")
                    print("True answer:",true_answer)
                    print("Predicted SQL:",decoded_pred )
                    ...
                    print("Shape:",predicted_token_ids.shape)

                    # # Decode the predicted token IDs into text
                    predicted_text = self.tokenizer.decode(predicted_token_ids[0].tolist(), skip_special_tokens=True)
                ...
                # pbar.update(task, advance=1 ,loss=-1)
...
def main():
    ...
    #tokenizer = AutoTokenizer.from_pretrained('/home/schatterjee1/KBLAM/Phi4-mini-ST')
    ...
    #print("Test check for special tokens",tokenizer.convert_ids_to_tokens(tokenizer( '<|begin_select|>SELECT<|end_select|> movie_title <|begin_from|>FROM<|end_from|> movies WHERE movie_release_year = 1945 ORDER BY movie_popularity DESC LIMIT 1', return_tensors="pt").to('cpu')['input_ids'].squeeze(0).tolist()))
    ...
    total_steps = np.ceil(len(dataset) / B).astype(int)
